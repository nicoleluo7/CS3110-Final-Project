  ...
  >::: [
         ...
         >:: (fun _ ->
               assert_equal ~printer:(fun x -> x) "A"
                 (prop_to_string (Var "A")));
         "prop_to_string and"
         >:: (fun _ ->
               assert_equal ~printer:(fun x -> x) "(A & B)"
                 (prop_to_string (And (Var "A", Var "B"))));
         "prop_to_string implication"
         >:: (fun _ ->
               assert_equal ~printer:(fun x -> x) "(A -> B)"
                 (prop_to_string (Imp (Var "A", Var "B"))));
         "prop_to_string negation"
         >:: (fun _ ->
               assert_equal ~printer:(fun x -> x) "!A"
                 (prop_to_string (Not (Var "A"))));
         "prop_to_string nested"
         >:: (fun _ ->
               assert_equal ~printer:(fun x -> x) "(!A & (B -> C))"
                 (prop_to_string
                    (And (Not (Var "A"), Imp (Var "B", Var "C")))));
       ]
  ...
  >::: [
         ...
         >:: (fun _ ->
               assert_equal ~printer:prop_to_string
                 ...
                 (parse_prop "(A & B) -> (C -> D)"));
         "parse negation of var"
         >:: (fun _ ->
               assert_equal ~printer:prop_to_string (Not (Var "A"))
                 (parse_prop "!A"));
         "parse negation of complex"
         >:: (fun _ ->
               assert_equal ~printer:prop_to_string
                 (Not (And (Var "A", Var "B")))
                 (parse_prop "!(A & B)"));
         ...
         "reject invalid variable"
         >:: (fun _ -> assert_parse_error "invalid var" "1");
         "reject unmatched paren"
         >:: (fun _ -> assert_parse_error "unmatched" "(A");
         "reject unmatched closing paren"
         >:: (fun _ -> assert_parse_error "unmatched close" "A)");
         "reject incomplete implication"
         >:: (fun _ -> assert_parse_error "incomplete imp" "A ->");
         "reject incomplete and"
         >:: (fun _ -> assert_parse_error "incomplete and" "A &");
       ]
...
let sequent_tests =
  ...
  >::: [
         ...
         "add_premise multiple"
         >:: (fun _ ->
               let st =
                 empty |> fun s -> add_premise s (Var "A") |> fun s -> add_premise s (Var "B")
               in
               assert_bool "Should contain A"
                 (List.mem (Var "A") st.premises);
               assert_bool "Should contain B"
                 (List.mem (Var "B") st.premises));
         "add_derived"
         >:: (fun _ ->
         ...
         "apply_modus_ponens simple"
         >:: (fun _ ->
               let st1 = add_premise empty (Var "A") in
               let st = add_premise st1 (Imp (Var "A", Var "B")) in
               let result = apply_modus_ponens st in
               assert_bool "Should derive B"
                 (List.mem (Var "B") result.derived));
         "apply_modus_ponens multiple steps"
         >:: (fun _ ->
               let st =
                 empty
                 |> fun s -> add_premise s (Var "A")
                 |> fun s -> add_premise s (Imp (Var "A", Var "B"))
                 |> fun s -> add_premise s (Imp (Var "B", Var "C"))
               in
               let result = apply_modus_ponens st in
               assert_bool "Should derive B"
                 (List.mem (Var "B") result.derived);
         ...
         "apply_modus_ponens no new derivations"
         >:: (fun _ ->
               let st = add_premise empty (Var "A") in
               let result = apply_modus_ponens st in
               assert_equal [] result.derived);
         "apply_modus_ponens bidirectional"
         >:: (fun _ ->
               let st =
                 empty
                 |> fun s -> add_premise s (Var "B")
                 |> fun s -> add_premise s (Imp (Var "B", Var "A"))
               in
               let result = apply_modus_ponens st in
               assert_bool "Should derive A from B -> A and B"
                 (List.mem (Var "A") result.derived));
         "judge_goal true when in premises"
         >:: (fun _ ->
               let st = add_premise empty (Var "A") |> fun s -> add_goal s (Var "A") in
               assert_bool "Goal should be judged true" (judge_goal st));
         "judge_goal true when in derived"
         >:: (fun _ ->
               let st =
                 empty
                 |> fun s -> add_premise s (Var "A")
                 |> fun s -> add_premise s (Imp (Var "A", Var "B"))
                 |> fun s -> add_goal s (Var "B")
               in
               let st = apply_modus_ponens st in
               assert_bool "Goal should be judged true" (judge_goal st));
         "judge_goal false when not found"
         >:: (fun _ ->
               let st = add_premise empty (Var "A") |> fun s -> add_goal s (Var "B") in
               assert_bool "Goal should be judged false" (not (judge_goal st)));
         "judge_goal false when no goal"
         >:: (fun _ ->
               let st = add_premise empty (Var "A") in
               assert_bool "Goal should be judged false" (not (judge_goal st)));
         "explain_derivation found"
         >:: (fun _ ->
               let st =
                 empty
                 |> fun s -> add_premise s (Var "A")
         ...
         "explain_derivation not found"
         >:: (fun _ ->
               let st = add_premise empty (Var "A") in
               match explain_derivation st (Var "B") with
               | Some _ -> assert_failure "Should not find derivation"
               | None -> ());
         "explain_derivation complex"
         >:: (fun _ ->
               let st =
                 empty
                 |> fun s -> add_premise s (Var "A")
                 |> fun s -> add_premise s (Imp (Var "A", And (Var "B", Var "C")))
               in
               let st = apply_modus_ponens st in
...
let suite =
  "logic prover tests"
  >::: [ast_tests; parser_tests; rule_tests; sequent_tests]
